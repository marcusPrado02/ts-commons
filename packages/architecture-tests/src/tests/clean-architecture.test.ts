...
describe('Clean Architecture Compliance', () => {
  ...
  beforeAll(() => {
    ...
    violations = analyzer.analyzeDependencies();
  });
  ...
  it('should have no critical dependency violations', () => {
    ...
    if (criticalViolations.length > 0) {
      const violationDetails = criticalViolations
        .map((v) => `${v.sourceLayer} -> ${v.targetLayer}: ${v.description}`)
        .join('\n');
  ...
  it('should minimize high severity violations', () => {
    ...
    if (highSeverityViolations.length > 0) {
      const violationDetails = highSeverityViolations
        .map((v) => `${v.sourceLayer} -> ${v.targetLayer}: ${v.description}`)
        .join('\n');

      console.warn(
        `High severity violations (${highSeverityViolations.length}/${maxAllowedHighViolations}):`,
      );
  ...
  it('should enforce domain layer isolation', () => {
    ...
    if (domainViolations.length > 0) {
      const violationDetails = domainViolations
        .map((v) => `Domain -> ${v.targetLayer}: ${v.description}`)
        .join('\n');
  ...
  it('should prevent circular dependencies', () => {
    ...
    if (circularDependencies.length > 0) {
      const cycleDetails = circularDependencies.map((cycle) => cycle.join(' -> ')).join('\n');
  ...
  it('should report architecture metrics', () => {
    ...
    for (const layer of layers) {
      ...
      console.info(
        `${layer.name} Layer: ${layer.components.length} components, ${layerViolations.length} violations`,
      );
    }
